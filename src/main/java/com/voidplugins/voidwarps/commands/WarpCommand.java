...
public class WarpCommand extends BaseCommand {
    ...
    public void onWarp(Player player, String warpName) {
        ...
        if (!config.contains("inventory.slots." + warpName.toLowerCase()) || 
            !config.contains("inventory.items." + warpName.toLowerCase())) {
            player.sendMessage("§cEsta warp não está configurada na config.yml");
            return;
        }
    ...
    public void onSet(Player player, String warpName) {
        ...
        if (!config.contains("inventory.slots." + warpName.toLowerCase()) || 
            !config.contains("inventory.items." + warpName.toLowerCase())) {
            player.sendMessage("§cEsta warp não está configurada.");
            return;
        }
    ...
    public void onDelete(Player player, String warpName) {
        ...
        if (!config.contains("inventory.slots." + warpName.toLowerCase()) || 
            !config.contains("inventory.items." + warpName.toLowerCase())) {
            player.sendMessage("§cEsta warp não está mais configurada na config.yml");
            return;
        }
